...
fy-NL ga-IE gd gu-IN he hi-IN hr hu id is it ja kk kn ko ku
...
IUSE="bindist oss4 gstreamer +jit +minimal pgo pulseaudio selinux system-cairo system-icu system-jpeg system-sqlite test"
...
src_configure() {
	...
	mozconfig_use_enable oss4 oss
	# patch for oss
	if use oss4; then
		epatch "${FILESDIR}"/palemoon-25.0.0-oss.patch
	fi
