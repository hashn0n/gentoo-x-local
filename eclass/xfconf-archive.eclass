...
# $Header: /var/cvsroot/gentoo-x86/eclass/xfconf-archive.eclass,v 1.41 2012/05/22 10:57:17 ssuominen Exp $

# @ECLASS: xfconf-archive.eclass
# @MAINTAINER:
...
# @ECLASS-VARIABLE: XFCONF-ARCHIVE
# @DESCRIPTION:
...
# @FUNCTION: xfconf-archive_use_debug
# @DESCRIPTION:
...
# If USE debug is enabled and the XFCONF-ARCHIVE_FULL_DEBUG variable is set, return --enable-debug=full.
xfconf-archive_use_debug() {
	if has debug ${IUSE}; then
		if use debug; then
			if [[ -n $XFCONF-ARCHIVE_FULL_DEBUG ]]; then
...
# @FUNCTION: xfconf-archive_src_unpack
# @DESCRIPTION:
# Run git-2_src_unpack if required
xfconf-archive_src_unpack() {
	NOCONFIGURE=1 git-2_src_unpack
}

# @FUNCTION: xfconf-archive_src_prepare
# @DESCRIPTION:
# Run base_src_prepare and eautoreconf or elibtoolize
xfconf-archive_src_prepare() {
...
# @FUNCTION: xfconf-archive_pkg_preinst
# @DESCRIPTION:
# Run gnome2_icon_savelist
xfconf-archive_pkg_preinst() {
	debug-print-function ${FUNCNAME} "$@"
	gnome2_icon_savelist
}

# @FUNCTION: xfconf-archive_pkg_postinst
# @DESCRIPTION:
# Run fdo-mime_{desktop,mime}_database_update and gnome2_icon_cache_update
xfconf-archive_pkg_postinst() {
	debug-print-function ${FUNCNAME} "$@"
...
# @FUNCTION: xfconf-archive_pkg_postrm
# @DESCRIPTION:
# Run fdo-mime_{desktop,mime}_database_update and gnome2_icon_cache_update
xfconf-archive_pkg_postrm() {
	debug-print-function ${FUNCNAME} "$@"
	fdo-mime_desktop_database_update
	fdo-mime_mime_database_update
	gnome2_icon_cache_update
